...
class ExperimentSetting2(Problem):
	"""
	Description: Simulates an autoregressive moving-average time-series.
	"""

	compatibles = set(['ExperimentSetting2-v0', 'TimeSeries'])
	...
	def initialize(self, max_T = 1000000000):
		...
		self.max_T = max_T
		self.n = 1
		self.a1 = np.array([-0.4,-0.5,0.4,0.4,0.1])
		self.a2 = np.array([0.6,-0.4,0.4,-0.5,0.4])
		self.b = np.array([0.32,-0.2])
		self.timeSeries = random.rand(len(self.a1))
		self.errorArray = np.empty(len(self.b))
		self.initError = random.uniform(-0.5,0.5,len(self.b))
		return self.step()

	def _alpha(self):
		return (self.a1 * (self.T / 10.0**4) + self.a2 * (1 - self.T / 10.0**4))
	...
	def step(self):
		...
		newValue = self.timeSeries.dot(self._alpha()) + self.errorArray.dot(self.b) + errorV
